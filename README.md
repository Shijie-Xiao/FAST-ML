...
member, giving a full intensity distribution to compare against IBTrACS —
and, for the 2025 cases, the Google WeatherLab FNV3 ensemble. Retaining the
ODE means the spread is generated by physics, so the ensemble keeps the
right-skewed tail that a model trained towards the conditional mean damps
out. On Hurricane Beryl (July 2024, part of the held-out season), the
sampled ensemble — 31 raw GEFS members expanded to 360 ODE members — yields
a full intensity distribution over the 5-day window:

![Beryl ensemble](img/tracks_beryl_ml.png)

*Beryl (2024, held-out): the sampled FAST-ML ensemble tracks. The GEFS
initialisation (2024-06-29 00Z) is triggered at the 60-kt threshold and
integrated freely.*
